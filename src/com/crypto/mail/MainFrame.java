...
public class MainFrame extends JFrame{
    ...
    byte[] key;
    byte[] encryptedKey;
    String encryptedByteKey;

    private final HashMap<String, JTextArea> map = new HashMap();
    private final HashMap<String, String> map2 = new HashMap();

    JButton calculateButton = new JButton("Добавить");
    private JTextField from_field = new JTextField("AliseTest228@gmail.com",30);
    ...
    public MainFrame() {
        ...
        Cipher publicKeyEncryption = null;
        try {
            publicKeyEncryption = Cipher.getInstance("RSA");
        ...
        try {
            encryptedKey = publicKeyEncryption.doFinal(key);
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            e.printStackTrace();
        }
        //String encryptedByteKey = Arrays.toString(encryptedKey);
        encryptedByteKey = new String(Base64.getEncoder().encode(encryptedKey));
        //key exchange end

        //---------------------------------------

        reciveMSG();
        newTab(User.username, key, encryptedByteKey);
    }

    private void newTab(final String addres, byte[]  key,  String encryptedByteKey){
        ...
        btn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                try {
                    sendMessage(addres, textAreaMsg, textAreaHist, key ,  encryptedByteKey);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        if(addres!=null)
            tabbedPane.setTabComponentAt(tabbedPane.getTabCount()-1,new ButtonTabComponent(tabbedPane,this,addres));
        tabbedPane.setSelectedIndex(tabbedPane.getTabCount()-1);

    }

    private void sendMessage(String recipient, JTextArea textAreaMsg, JTextArea textAreaHist, byte[]  key,  String encryptedByteKey) throws Exception {
        ...
        Session session = Session.getDefaultInstance(props,
                new javax.mail.Authenticator() {
                    protected javax.mail.PasswordAuthentication getPasswordAuthentication() {
                        return new javax.mail.PasswordAuthentication(User.username, User.password);
                    }
                });
        String recipient = r;
                ...
                try {
                    TimeUnit.SECONDS.sleep((1));
                        ...
                        for (int i = endi; i < messages.length; i++) {
                            ...
                            procMSG(message.getFrom()[0].toString(),descrypmess);
                    ...
                    JOptionPane.showMessageDialog(MainFrame.this, "connection failure",
                            "at com.crypto.mail.MainFrame$6.run(MainFrame.java:300)", JOptionPane.WARNING_MESSAGE);
                    reconect();
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(MainFrame.this, "63",
                            "at com.crypto.mail.MainFrame$6.run(MainFrame.java:300)", JOptionPane.WARNING_MESSAGE);
                    e.printStackTrace();
                }
            }
    ...
    private void procMSG(String from, String msg){

        if(User.username.equals(from)){
            tabbedPane.setSelectedIndex(0);
            writeText(forme, from,msg);
            return;
        }

        JTextArea t = map.get(from);
        if(t!=null){
            tabbedPane.setSelectedIndex(findInTabbed(from));//!
        }
        else{
            newTab(from,  key, encryptedByteKey);
            t = map.get(from);
        }
        writeText(t,from,msg);
    ...
    private void reconect(){
        try {
            MainApp.store.connect("imap.gmail.com", User.username, User.password);
    ...
    private void init() { // Приватный метод, где работаем с окном
        ...
        setLayout(null);
        setTitle("Чат, ваш логин: "+ User.username);
    ...
    private void Box() { // Приватный метод, где работаем с окном
        ...
        from_field.setSize(203,20);
        this.add(from_field);

        calculateButton.setSize(203,20);
        ...
        this.getContentPane().addHierarchyBoundsListener(new HierarchyBoundsListener() {
            public void ancestorResized(HierarchyEvent arg0) {
                ...
                from_field.setLocation(getWidth()/2-120,15);
                calculateButton.setLocation(getWidth()/2-120,45);
        ...
        calculateButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent) {
                listModel.addElement(from_field.getText());

                newTab(from_field.getText(), key, encryptedByteKey);
            }
        });
        ...
        list.addListSelectionListener(new ListSelectionListener() {
            public void valueChanged(ListSelectionEvent arg0) {
                ...
                newTab(a[0], key, encryptedByteKey);
            }
        });
